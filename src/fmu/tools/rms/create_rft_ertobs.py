...
This script should be run from within RMS::
 ...
 * inputfile (str): Path to CSV file with columns: WELLNAME, ...(ADDHERE)
 * exportdir: Path, typically relative to rms/model
 * alias: dictionary mapping RMS welll names to Eclipse names, or string with
   CSV file.
...
# Default observation error (if not specified). Given as an absolute value (3
# bars by default).  It could also be defined as a ratio of the observation
# value (2% or 5% for example).  absolute_err: True for absolute error, False
# for relative error as percentage of the RFT value.
absolute_error = True
obs_error = 3.0
...
# To be used if the well_name in input_file correspond to the standard well name
# in RMS instead of the duplicated well associated with RFT data.  If the name
# used in the input_file already includes it, then use: rft_prefix = '' The
# final RMS well_name will be considered to be "rft_prefix + <well_name from
# input_file>"


def make_alias_dict(alias_file, rms_name, ecl_name):
    """
    Create a correspondance dictionary so that
    well_dict["rmsname] = "eclname"
    """
...
def get_well_coords(project, wellname, trajectory_name="Drilled trajectory"):
    """Return well coordinates as a (wellpoints, 3) numpy array.
...
def strictly_downward(coords):
    # Check if the well is strictly going down.
    for j in range(0, len(coords[:, 3]) - 1):
        if coords[j + 1, 3] < coords[j, 3]:
            return False
    return True


def interp_from_md(md_value, coords, interpolation="cubic"):
    """
...
def interp_from_xyz(xyz, coords, interpolation="cubic"):
    ...
    if strictly_downward(coords):
        ...
        if interpolation.lower() == "linear":
            md_value = np.interp(z0, coords[:, 3], coords[:, 0])
        else:
            md_value = float(CubicSpline(coords[:, 3], coords[:, 0])(z0))
        logger.info("MD estimated on strictly downward wellpath: {}".format(md_value))
        return md_value
...
def check_config(config):
    ...
    if missing_keys:
        logger.error("Missing configuration: %s", str(missing_keys))
        return

    required_input_file_columns = ["DATE", "MD", "WELL_NAME", "VALUE"]
    ...
    if missing_columns:
        logger.error(
            "Missing columns %s in input_file %s",
            str(missing_columns),
            config["input_file"],
        )
        return
    ...
    if "project" not in config:
        # If we are running inside RMS, the project variable is defined:
        if "project" in vars():
            logger.debug("Obtained RMS project reference from context")
            config["project"] = project
        else:
            logger.error("RMS project is not defined.")
            logger.error(
                "If you are running external to RMS, you must provide a mocked RMS project"
            )

    if "alias" not in config:
        config["wellalias"] = {}  # Empty dictionary is the default
    if "alias_file" in config:
        logger.info("Getting RMS to Eclipse aliases from %s", config["alias_file"])
        df = pd.read_csv(alias_file, index_col=config.get("rms_name", 0))
        config["alias"] = df.to_dict()[config.get("ecl_name", 1)]
    return config
...
def main(config):
    ...
    for well in well_names:
        ...
        for i in range(0, n_obs):
            ...
            if not xyz:
                assert is_md, (
                    "MD must be specified as input "
                    "if (East, North, TVD) is not given!"
                )
                # Compute X,Y,Z
            ...
            cell_index = grid.get_cells_at_points(
                [obs["EAST"], obs["NORTH"], obs["TVD"]]
            )
            ...
            if not pd.isnull(obs["ZONE"]):
                ...
                if obs["ZONE"] == zone_str:
                    ...
                    print(
                        ...
                        "grid zone ({}) at given coordinates!".format(
                            obs["ZONE"], zone_str
                        )
                    )
